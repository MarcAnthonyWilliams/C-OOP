...
class PlainOldClassV2 {  
    public:
        ...
        int getX() const { return x; }
        //void setX( int x )  { x = x; } // HMMMM??? --> Prolly setting same value to itself?
...
// Task 29 SpeakerSystem definition and Amplifier
...
private:
   const SpeakerSystem* attachedSpeakers = nullptr; // originall bad pointer casting.
   //const before means cant change data.
};
...
public:       
    ...
    void movesInWith(Person& newRoomate) {
        if(haveRoomie() == false){
            ...
            cerr << "You already have a roomate! " << endl;
        }      
    }       
...
int main() {
    ...
    // it. Observe the value of x. 
        //--> garbage on instantiation, assign aftre assign
    // [No new code] 
    ...
    //p = 0x7fffffffdcec;
    //a value of type "long" cannot be assigned to an entity of type "int *"
    ...
    *p = -2763;  
    ...
    //Remember p points to adress of x, so deref p and x == same thing (-2763)
    ...
    *p = 980;      
    ...
    int* q;      
    q = p;  
    //q now points to p's address, and they both can access the data
    //stored their    
    ...
    *p  = *pD; //73.2343
    *q  = *p;  
    //pD  = p; Pointer casting of diff types cCANNOT happen
    //p   = pD;
    ...
    pI = &joe;  
    *pI = 234;  
   // pI = &sal;  //Bad pointer casting. pI not const int 
    *pI = 7623;    

    const int* pcI; // can't change data
    pcI = &joe;  
   // *pcI = 234; // cant change pCI's val 
    pcI = &sal;  
   // *pcI = 7623; // cant change pCI's val 

    //int* const cpI; // we are saying cPI address can't change, so it needs to have that value already that it can't change from.
    //can't change addy.
    int* const cpI(&joe);
    //int* const cpI(&sal); // bad pointer casting. This can change values
    //cpI = &joe;  // same stuff
    *cpI = 234;  // can change our value though, as it doesnt change addy
    //cpI = &sal;  
    *cpI = 7623;    

    //const int* const cpcI; // how can you say not to change something thta hasnt been given any information on WHAT not to change?
    const int* const cpcI(&joe);
    //const int* const cpcI(&sal); //Not sure why this compules
    //cpcI = &joe;  *cpcI = 234;  neither wil work.
    //cpcI = &sal;  
    //*cpcI = 7623; 
    ...
    *pDyn = 17;
    ...
    cout << pDyn << endl;  
    delete pDyn;  //nothing happened? lol
    cout << pDyn << endl;
    ...
    //Seg fault cout << "Can we dereference nullptr?  " << *pDyn << endl;      
    ...
    //delete jumper; //expression must be a pointer to a complete object typeC/C++(852)
    ...
    aux_cord->attachSpeakers(speaker);
    aux_cord->playMusic();
